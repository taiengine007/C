...
void MAX6675_ReadRawData(MAX6675 *struc);   //Reads the raw temperature data from the sensor over SPI.
uint16_t MAX6675_ConvertRawDataToTempIntDeg(MAX6675 *struc);//Returns temperature as an integer.
float MAX6675_ConvertRawDataToTempFloatDeg(MAX6675 *struc);//Returns temperature as a floating-point value.
uint8_t MAX6675_CheckOpenThermocouple(MAX6675 *struc);  //Checks if the thermocouple is open (disconnected or broken).
