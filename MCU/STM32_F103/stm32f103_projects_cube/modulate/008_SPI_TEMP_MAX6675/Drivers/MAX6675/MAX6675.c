...
void MAX6675_ReadRawData(MAX6675 *struc){
	...
	printf("hal gpio writepin\n");
	 if (HAL_SPI_Receive(struc->spiHandle, (uint8_t *)&rawData, 2, 100) != HAL_OK) {
	        // Error in communication
	        printf("SPI receive failed\n");
	        return;
	    }

	printf("spiiiiiii---receive--\n");


	HAL_Delay(10); // Small delay to stabilize after reading


    // Deassert chip select high
     HAL_GPIO_WritePin(struc->GPIO_PORT_PIN, struc->GPIO_Pin, GPIO_PIN_SET);

     // Store the raw data in the structure
  	struc->rawSPIdata = rawData;
  	// Debug: Print the received raw data (optional)
