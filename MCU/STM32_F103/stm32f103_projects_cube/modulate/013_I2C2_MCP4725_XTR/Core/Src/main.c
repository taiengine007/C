...
// Triangle wave generation variables
uint16_t dac_value = 0;
uint16_t step_size = 10;       // Step size for the triangle wave
uint16_t max_dac_value = 4095; // 12-bit DAC max value
  ...
  while (1)
  {
    ...
    /* USER CODE END WHILE */
	  ...
	  /*
	   ...
	   MCP4725_setVoltage(&myMCP4725,3.2f, MCP4725_FAST_MODE, MCP4725_POWER_DOWN_OFF);
	   printf("3.2f  3sec\n");


	   HAL_Delay(10000);

	   MCP4725_setVoltage(&myMCP4725,1.2f, MCP4725_FAST_MODE, MCP4725_POWER_DOWN_OFF);
	   printf("1.2f  3sec\n");
	  ...
	  /*
	     // Set the DAC value
	        MCP4725_setValue(&myMCP4725, dac_value, MCP4725_FAST_MODE, MCP4725_POWER_DOWN_OFF);

	        printf("dac value --- %u\n",dac_value);

	        // Update DAC value for the next step
	        dac_value += step_size * direction;
	        ...
	        // Delay to control the frequency of the wave
	        HAL_Delay(1); // Adjust for the desired wave frequency
	    }
